...
import { RootState } from "../../../core/data/interface";
...
const AdminDashboard = () => {
  ...
  // Access dashboard data from Redux store (if you're storing dashboard data)
  ...
  // Fetch data from Google Sheets API on mount
  console.log("-----------------------------------",candidates)
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<{ values: any[] }>('http://localhost:3002/api/google-sheets-data/');
        
        // Assuming the response structure has 'values' which is an array of rows
        const sheetData = response.data.values;

        // Skip the first row (header row) and map the data
        const formattedData: Candidate[] = sheetData.slice(1).map((row: any[]) => ({
          Cand_ID: row[0],            // Candidate ID (from Google Sheets)
          Candidate: `${row[0]} ${row[1]}`,  // Combine First and Last name to form 'Candidate'
          Email: row[11],             // Email column
          Applied_Role: row[2],       // Applied Role
          Phone: row[12],             // Phone
          Applied_Date: row[15],      // Applied Date
          Status: row[3],             // Status
          Image: '',                  // Image URL (if any)
          Resume: row[13]             // Resume file link
        }));

        setCandidates(formattedData); // Set the data in state
      } catch (err) {
                ...
                <div className="col-md-3 d-flex">
                  <div className="card flex-fill">
                    <div className="card-body">
                      ...
                      <Link to="/candidates-grid/ " className="link-default">
                        View All
                      </Link>
                    </div>
                  </div>
                ...
                <div className="col-md-3 d-flex">
                  <div className="card flex-fill">
                    <div className="card-body">
                      ...
                      <Link to="/candidates" className="link-default">
                        View All
                      </Link>
                    </div>
                  </div>
