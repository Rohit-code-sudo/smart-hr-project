...
interface Candidate {
  ...
  const CandidateGrid = () => {
    const [candidates, setCandidates] = useState<Candidate[]>([]); // State to store candidate data
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          console.log("fasjkjhlkjh-------------",response)
          // Assumin the response data structure has a 'values' array
          const sheetData = response.data.values;
  
          // Map sheet data to match the Candidate interface
          const formattedData: Candidate[] = sheetData.map(row => ({
            Cand_ID: row[0],
            Candidate: row[1],
            Email: row[2],
            Applied_Role: row[3],
...
console.log("grid daata ----------------------",candidates)
    return (
        <>
            {/* Page Wrapper */}
